defmodule LiveStash.Adapters.BrowserMemory.SerializerTest do
  ...
  describe "serialization round-trip (term_to_external and external_to_term)" do
    test "works correctly in :sign mode", %{socket: socket} do
      ...
      token = Serializer.term_to_external(socket, original_state, opts)
      assert is_binary(token)

      assert {:ok, ^original_state} =
               Phoenix.Token.verify(socket, opts.secret, token, max_age: opts.ttl)

      assert {:ok, ^original_state} = Serializer.external_to_term(socket, token, opts)
    end
  ...
  describe "error handling" do
    test "returns an error if token cannot be decoded or is malformed", %{socket: socket} do
      ...
      assert {:error, :invalid} =
               Serializer.external_to_term(socket, "invalid_or_malformed_token", opts)
    end

    test "returns an error for expired tokens", %{socket: socket} do
      opts = %{security_mode: :sign, secret: "my_secret", ttl: 1000}

      token = Serializer.term_to_external(socket, %{time_test: "data"}, opts)

      Process.sleep(2000)

      assert {:error, :expired} = Serializer.external_to_term(socket, token, opts)
    end
  end
end
