defmodule LiveStash.Adapters.ContextTest do
  ...
  for adapter <- @server_adapters do
  ...
  describe "#{inspect(@browser_memory)}.new/3" do
    test "returns settings when security_mode is provided", %{socket: socket} do
      session = %{}
      opts = [ttl: 1, stored_keys: [:username], security_mode: :sign]

      context = @browser_memory.new(socket, session, opts)

      assert context.__struct__ == @browser_memory
      assert context.secret == @default_secret
      assert context.ttl == 1
      assert context.reconnected? == false
    ...
    test "fetches, hashes, and base64 encodes secret from session when session_key is provided",
      ...
      opts = [session_key: "my_stash_key", stored_keys: [:username], security_mode: :encrypt]
      ...
      assert context.secret == expected_secret
      assert context.security_mode == :encrypt
    end

    test "raises ArgumentError when session_key is not found in session", %{socket: socket} do
      session = %{"other_key" => "value"}

      opts = [session_key: "missing_key", stored_keys: [:username], security_mode: :sign]
    ...
    test "raises ArgumentError when session_key returns a non-binary value", %{socket: socket} do
      ...
      opts = [session_key: "int_key", stored_keys: [:username], security_mode: :sign]
    ...
    test "reraises a custom RuntimeError when LiveView.get_connect_params/1 fails" do
      broken_socket = %Socket{transport_pid: self()}
      session = %{}
      opts = [stored_keys: [:username], security_mode: :sign]

      assert_raise RuntimeError, ~r/Failed to get connect params/, fn ->
        @browser_memory.new(broken_socket, session, opts)
      end
    end
